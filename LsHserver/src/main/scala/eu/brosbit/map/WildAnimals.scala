  ...
  def feed():Unit = {
    deerArr.foreach(d => {
      ...
      if(plant.getBio >= toEat) {
        ...
        d.eaten = (d.eaten + toEat.toShort).toShort
      } else {
 ...
 */
  ...
  private def freePlain(c: Int, r: Int): Boolean = {
    ...
    private def isWood(c:Int, r:Int): Boolean = {
      val obj = map.getTile(r, c).imObjOpt
      if(obj.isEmpty) return false
      else if obj.get.build then return false
      else if obj.get.plant.obj.shortName == Forest.shortName then return true
      else return false
    }
  ...
  private def isGrass(c:Int, r:Int): Boolean = {
    ...
    else if obj.get.plant.obj.shortName == Grass.shortName then return true
    else return false
  }
