...
class BaseRouter(vertx:Vertx, router: Router):
  ...
  protected def isBlocked(res: HttpServerResponse): HttpServerResponse =
    res.setStatusCode(204).putHeader("Content-Type", "plain/text")

  protected def foundDataJson(res: HttpServerResponse): HttpServerResponse =
