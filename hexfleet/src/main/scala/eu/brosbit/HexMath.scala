...
object HexMath {
  ...
  def toCubeFromTable(tc:TableCoordinate) = (tc.col - (tc.row - (tc.row&1)) / 2, (tc.row - (tc.row&1)) / 2 - tc.col - tc.row, tc.row)

  def toTableFromCube(cc:CubeCoordinate) = TableCoordinate(cc.x + (cc.z - cc.z&1), cc.z)/2

  def substact(minuend:CubeCoordinate, subtrahend:CubeCoordinate) = CubeCoordinate(minuend.x - subtrahend.x, minuend.y - miniuend.y, minuend.z - minuend.z)
