...
class MapGenerator(val sizeXY: Int) {
  ...
  val worldTiles = Array.ofDim[Char](sizeXY, sizeXY).map(row => row.map(x => 'p'))
  println("Size of map: " + this.sizeXY * this.sizeXY);

  generateMap()
  
  def getMapJson() = "[" + worldTiles
    .map(line => "[" +line.map(a => "\""+a.toString+"\"").mkString(", ") + "]")
    .mkString(", ") + "]"
  def getMap:Array[Array[Char]] = worldTiles
  ...
  */
  ...
  private def mkSideRivers(sideRiverStart: List[HexPoint]): Unit = {
    ...
    for (a <- sideRiverStart) {
      ...
      if (random.nextDouble() > weightRandom) {
        r = 1
        weightRandom += 0.15
      } else {
        r = -1
      ...
      while (sideRiverLong > 0) {
        ...
        if (upDownOrLeft == 0 || lastUpDownOrLeft == -upDownOrLeft) {
          COL += r
          if (!(COL < 0 || COL >= this.sizeXY || ROW < 0 || ROW >= this.sizeXY)) this.worldTiles(COL)(ROW) = waterType
        } else if (ROW % 2 == 0) {
          ROW += upDownOrLeft
          if (r < 0) COL += r
          if (!(COL < 0 || COL >= this.sizeXY || ROW < 0 || ROW >= this.sizeXY)) this.worldTiles(COL)(ROW) = waterType
