...
class LoadTilesConfigSpec extends AnyFlatSpec with Matchers {
 ...
 /* "Manager" should "return None when unnamed titles apears" in {
  ...
  "Manager" should "find proper title named Hill with rigth values" in {
    val hillOpt = TilesCreateManager.getTile("Hill")
    assert(hillOpt.isDefined, true)
    val hill = hillOpt.get
    assert(hill.getaType === 30.asInstanceOf[Byte])
