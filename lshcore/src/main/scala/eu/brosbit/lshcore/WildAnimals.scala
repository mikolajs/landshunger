...
import eu.brosbit.lshcore.immovable.{Forest, Grass, Plant}
import eu.brosbit.lshcore.movable.{Boar, Deer, Wisent}
  ...
  private def findNewDestinationNearOrElseFar(d:Deer):Unit = {
    ...
    /*
      val position = (d.position::hexlib.neighbours(d.position))
        ...
        .filter(mp => !mapArray(mp.r)(mp.c)).max
y
  ...
  private def canPlaceAnimal(c: Int, r: Int, animal:String) =
    animal match
      case a if Deer.name == a => canPlaceDeer(c, r)
      case a if Wisent.name == a => canPlaceWisent(c, r)
      case a if Boar.name == a => canPlaceBoar(c, r)
      
  private def canPlaceDeer(c:Int, r:Int): Boolean =
    freeAndPlainOrHill(c, r) && (isWood(c, r) || isGrass(c, r))
  private def canPlaceWisent(c:Int, r:Int): Boolean =
    freePlain(c,r) || isGrass(c,r)
  private def canPlaceBoar(c: Int, r:Int): Boolean =
